...
def create_product(input: schemas.ProductInput, db: Session = Depends(get_db)):
    return crud.create_product(db, **input.model_dump())


@app.get("/products/{pk}", response_model=schemas.Product, tags=["products"])
def get_product(pk: str, db: Session = Depends(get_db)):
    return crud.get_product(db, pk)
